...
class BinaryTree:
    ...
    def delete(self, value):
        ...
        if idx == -1:
            messagebox.showinfo("Info", f"Value {value} not found in tree.")
...
class BinaryTreeApp:
    def __init__(self, root):
        ...
        # Title label
        title_label = tk.Label(root, text="Binary Trees Using Arrays", font=("Arial", 16, "bold"))
        ...
        self.search_button = tk.Button(control_frame, text="Search", command=self.search_value)
        self.search_button.pack(pady=5)

        # Clear Tree button
        self.clear_button = tk.Button(control_frame, text="Clear Tree", command=self.clear_tree)
        ...
        self.search_result_label = tk.Label(control_frame, text="")
    ...
    def search_value(self):
        value = int(self.value_entry.get())
        idx = self.tree.search(value)
        if idx != -1:
            self.search_result_label.config(text=f"Value {value} found at index {idx}.")
        else:
            self.search_result_label.config(text=f"Value {value} not found.")
        self.value_entry.delete(0, tk.END)
